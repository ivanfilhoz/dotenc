...
export const getToken = async (environment: string) => {
	...
	const tokensFile = path.join(os.homedir(), ".safe-env", "tokens.json")
	...
	throw new Error(
		"No token found. Please set the TOKEN environment variable or import the token using `npx safe-env import-token -e <environment> <token>`.",
	)
}
